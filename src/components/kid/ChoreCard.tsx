...
interface Props {
  ...
  onSubmit: (input: { photoFile?: File; note?: string }) => Promise<void>
}
...
export default function ChoreCard({ chore, completion, onSubmit }: Props) {
  ...
  async function submit() {
    if (!photoFile && !note.trim()) {
      setError('Add a photo or a note before submitting.')
      return
    }
    setBusy(true)
    setError(null)
    try {
      await onSubmit({ photoFile: photoFile ?? undefined, note: note.trim() || undefined })
      setSheetOpen(false)
    } catch (err) {
      setError(err instanceof Error ? err.message : "Couldn't submit that, try again.")
    } finally {
      setBusy(false)
    }
  }
  ...
  const badgeCls =
    status === 'pending' ? 'text-amber-300' : status === 'rejected' ? 'text-rose-300' : 'text-white/40'
    ...
    <motion.div layout className="glass rounded-2xl p-4">
      ...
      >
        ...
        <div className="min-w-0 flex-1">
          <div className={`font-medium ${status === 'approved' ? 'line-through text-white/50' : ''}`}>{chore.title}</div>
          ...
          {sheetOpen && (
            ...
            >
              ...
              >
                ...
                <p className="mt-1 text-xs text-white/40">Add a photo or a note as proof — at least one.</p>

                <button
                  onClick={() => photoInputRef.current?.click()}
                  className={`mt-4 flex w-full flex-col items-center gap-1 rounded-xl py-3 text-xs transition ${
                    photoFile ? 'bg-fuchsia-500/25 text-fuchsia-200 ring-2 ring-fuchsia-400/60' : 'bg-white/5 text-white/60 hover:bg-white/10'
                  }`}
                >
                  <Camera size={18} /> {photoFile ? 'Photo added' : 'Photo'}
                </button>

                {photoPreview && <img src={photoPreview} alt="" className="mt-3 h-32 w-full rounded-xl object-cover" />}
                ...
                <div className="mt-4 flex gap-2">
                  ...
                  >
                    {busy ? <Loader2 size={15} className="animate-spin" /> : 'Submit for approval'}
                  </button>
                </div>
              </motion.div>
            </motion.div>
      ...
      <AnimatePresence>
        {viewing && completion && <ProofModal completion={completion} chore={chore} onClose={() => setViewing(false)} />}
      </AnimatePresence>
    </motion.div>
  )
}
